...
For desktop rendering, use PySide6 image source cropping with these pixel rectangles and
nearest-neighbor scaling. Preserve full art and keep labels separate. Do not use
fill-cropping for character images. Seated Fairy's transparent canvas includes padding;
align the actual seat to wood, not the image's bottom. In the reference HTML it is an
overlay anchored to the last slot, with feet below the board. Charms use bottom-aligned
source crops touching the board. Shelf geometry is CSS with warm wood colors and simple
brackets, reproducible with replaceable PySide6 drawing tokens; no custom image processing
is required.
