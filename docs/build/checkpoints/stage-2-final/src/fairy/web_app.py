import hmac
from pathlib import Path

from fastapi import Depends, FastAPI, Header, HTTPException, status
from fastapi.staticfiles import StaticFiles
from pydantic import BaseModel

from fairy.neurofairy_chat import reply_to_user

PROJECT_ROOT = Path(__file__).resolve().parents[2]
UIUX_DIR = PROJECT_ROOT / "uiux"
UI_DIR = UIUX_DIR / "ui"


class ChatRequest(BaseModel):
    message: str


def create_app(*, api_token: str | None = None) -> FastAPI:
    app = FastAPI(title="Neurofairy")

    def require_desktop_token(x_fairy_token: str | None = Header(default=None)) -> None:
        if api_token is None:
            return
        if x_fairy_token is None or not hmac.compare_digest(x_fairy_token, api_token):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="desktop API token is missing or invalid",
            )

    protected = [Depends(require_desktop_token)]

    @app.get("/api/health", dependencies=protected)
    def health() -> dict[str, str]:
        return {"service": "neurofairy", "status": "ok"}

    @app.post("/api/chat", dependencies=protected)
    def chat_with_neurofairy(request: ChatRequest) -> dict[str, str]:
        message = request.message.strip()

        if not message:
            return {"reply": "I’m here. Whenever you’re ready."}

        try:
            return {"reply": reply_to_user(message)}
        except Exception:
            return {
                "reply": (
                    "I'm having trouble thinking clearly. Try again later. "
                    "For now, choose one small action."
                )
            }

    app.mount("/assets", StaticFiles(directory=UIUX_DIR / "assets"), name="assets")
    app.mount("/", StaticFiles(directory=UI_DIR, html=True), name="ui")
    return app


app = create_app()
